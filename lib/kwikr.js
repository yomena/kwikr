...
function convertThis() {
    if(process.argv.length > 2) {
        ...
        if (process.argv[2] === "new"){
            ...
            console.log('Hi, welcome to the kwikr, let\'s walk together through the setup process to create ' + projectName.yellow + '.');
            ...
            inquirer.prompt(questions).then(function (answers) {
                ...
                npm.install({save: true, cwd: projectName}, function(err) {
                    if (err) {
                        ...
                        spinner.stop(true);
                        console.log('Installation succeeded! You created '.green + projectName.green + '.'.green);
                    }
                });
            });
        }
